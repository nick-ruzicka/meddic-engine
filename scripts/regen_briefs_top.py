#!/usr/bin/env python3
"""Regenerate account briefs for the top-N scored contacts so new MEDDIC
field labels propagate."""
...
def main(limit: int = 10) -> int:
    ...
    rows = conn0.execute("""
        SELECT s.id AS score_id, s.score, s.label, s.action, s.reasoning, s.missing,
         ...
         WHERE s.id IN (
             SELECT MAX(id) FROM scores GROUP BY contact_id
         )
    ...
    print(f"Regenerating MEDDIC briefs for top {len(rows)} contacts")
    ok = fail = 0
    for r in rows:
        conn = get_db()
        sig = conn.execute("""
        ...
        if brief:
            for attempt in range(10):
                try:
                    ...
                    time.sleep(1.5)
            ok += 1
            print(f"  ✓ {r['firm_name']} / {r['contact_name']} ({r['score']})")
        else:
            fail += 1
