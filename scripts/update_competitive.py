...
def _get_v2_signals(conn) -> list[dict]:
    """Query ci_signals for recent actionable signals. Returns empty list if table doesn't exist."""
    try:
        v2_rows = conn.execute("""
            SELECT * FROM ci_signals
            WHERE category IS NOT NULL AND category != 'noise'
            ORDER BY predictive_score DESC, observed_at DESC
            LIMIT 100
        """).fetchall()
    except Exception:
        return []
...
def build_competitive_data() -> dict:
    ...
    try:
        ...
        # v2 signals from ci_signals table
        v2_signals = _get_v2_signals(conn)
        ...
        return {
            ...
            "stats": {
                ...
                "v2_signals_total":  len(v2_signals),
            },
...
def main() -> int:
    ...
    print(
        ...
        f"{s.get('v2_signals_total', 0)} v2 signals"
    )
    return 0
